...
- `cadreur-rs/` — the **Rust rewrite** of the Mac app. Feature-complete and
  verified against the Python, but NOT yet deployed to the venue. Ships as one
  signed 1.5 MB `Cadreur.app` with no interpreter, no venv and no repo folder
  beside it. `scripts/build_app.sh` builds, signs and notarizes it.

The Python app in `src/cadreur/` is still the one that runs the show. Do not
change it in order to suit the Rust port; the port exists to match it.
...
- `cd cadreur-rs && cargo test` — 100 unit tests plus the parity tests. Keep
  clippy clean (`cargo clippy --all-targets`).
- Toolchain is pinned in `cadreur-rs/rust-toolchain.toml`.
- **The web UI is not duplicated.** `api.rs` embeds `src/cadreur/web/` directly,
  so both builds serve the same files. Edit the UI in one place.
- **Never call `prompt()`, `confirm()` or `alert()` in the web UI.** wry's
  WKWebView implements only the file-open panel, so those are silent no-ops in
  the Rust build: the dialog never appears, `prompt()` returns null and
...
- **Diagnostics must go through `log_line!`,** never `eprintln!`: a bundle
  ...
  the log file.
- **Parity is the contract.** `tests/fixtures/` holds outputs generated by the
  Python; `tests/parity_api.py` diffs every REST endpoint between the two
  running implementations. Regenerate fixtures only when the Python reference
  intentionally changes — never to make a failing Rust test pass.
